...
**ドロネー三角形分割の前にプロジェクタ座標ごとにカメラ座標を集約する**（既定は成分ごとの中央値）。

`LinearNDInterpolator` は重複入力点を**平均化しない**（内部の QHull が重複頂点を coplanar として
最小インデックスの 1 点のみ採用し、残りを黙って捨てる）。そのため `_aggregate_duplicate_points` で
明示的に集約してから一意な入力点として渡す（中央値はデコード外れ値に頑健）。
