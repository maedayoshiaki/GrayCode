...
**ドロネー三角形分割の前にプロジェクタ座標ごとにカメラ座標を集約する**
（`_aggregate_duplicate_points`、既定は成分ごとの中央値。デコード外れ値に頑健）。

`LinearNDInterpolator` は重複入力点を**平均化しない**。内部の QHull が重複頂点を
coplanar として最小インデックスの 1 点のみ採用し残りを黙って捨てるため、明示的な
事前集約が必要である（旧版はこれを欠き、step $>1$ で多数の対応点が黙殺されていた）。
