"""座標系・GT保存性の回帰テスト (HIGH 修正 H1/H2/H3/H4 を固定する)。

規約: カメラ XY は整数=ピクセル中心、プロジェクタ UV は中心=i+0.5。
詳細は warp_image.py docstring / FEATURES.md / test_capture_seam.py を参照。
"""
...
# ── H2: P2C 補間が GT を保存し、出力 proj 列が半整数中心になる ──────────


def test_p2c_preserves_gt_on_halfinteger_grid() -> None:
    H = W = 16
    ...
    for j in range(H):
        for i in range(W):
            if rng.random() < 0.8:  # 部分的な復号被覆を模擬
                px, py = i + 0.5, j + 0.5  # decode 規約 (ブロック中心, step=1)
    ...
    # 出力 proj 座標列は UV 中心 (i+0.5) 規約
    assert np.allclose(out[5, 7, 0], 7.5)
    assert np.allclose(out[5, 7, 1], 5.5)
...
def test_p2c_duplicates_not_silently_dropped() -> None:
    ...
    # proj 画素(1,1) (中心 1.5,1.5) のカメラ値は 2 観測の中央値 = 15
    assert np.allclose(out[1, 1, 2], 15.0)
    assert np.allclose(out[1, 1, 3], 15.0)


# ── H3: forward_warp の bilinear が UV 中心の点を単一画素に載せる ──────


def test_forward_bilinear_centers_on_single_pixel() -> None:
    # cam(2,2) -> UV(3.5,3.5) = UV 画素(3,3) の中心
    pmap = np.array([[2.0, 2.0, 3.5, 3.5]], dtype=np.float32)
...
def test_forward_backward_roundtrip_identity() -> None:
    ...
    # 恒等幾何: cam(x,y) -> UV(x+0.5, y+0.5)
    rows = np.stack(
        [
            xs.ravel().astype(np.float32),
            ys.ravel().astype(np.float32),
            xs.ravel().astype(np.float32) + 0.5,
            ys.ravel().astype(np.float32) + 0.5,
        ],
        axis=1,
    ).astype(np.float32)
...
# ── H4: gen の step 引数順が decode と一致 (height_step, width_step) ──
...
def test_gen_cli_step_arg_order(monkeypatch) -> None:
    ...
    # CLI: H W height_step width_step
